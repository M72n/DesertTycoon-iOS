...
struct ContentView: View {
    ...
    var body: some View {
        GeometryReader { geometry in
            ...
            ZStack {
                BundleImage(candidates: [
                    "iphone-hd-upscaled/splash/dt_splash_arabic.jpg",
                    "iphone-hd/splash/dt_splash_arabic.jpg",
                    "preview_splash_arabic.jpg"
                ])
                    .scaledToFill()
    ...
    private var stagePanel: some View {
        ZStack(alignment: .bottomLeading) {
            BundleImage(candidates: selectedPanel == .map ? selectedPhase.mapAssetCandidates : [
                "iphone-hd-upscaled/souk_screen/souk_bg.jpg",
                "iphone-hd/souk_screen/souk_bg.jpg",
                "preview_souk_bg.jpg"
            ])
                .scaledToFill()
                .frame(maxWidth: .infinity)
...
private enum GamePhase: String, CaseIterable, Identifiable {
    ...
    var mapAssetCandidates: [String] {
        switch self {
        case .phaseOne:
            return [
                "iphone-hd-upscaled/maps/DT_Phase1_ortho.png",
                "iphone-hd/maps/DT_Phase1_ortho.png",
                "preview_phase1_map.png"
            ]
        case .phaseTwo:
            return [
                "iphone-hd-upscaled/maps/DT_Phase2_ortho.png",
                "iphone-hd/maps/DT_Phase2_ortho.png",
                "preview_phase2_map.png"
            ]
        case .phaseThree:
            return [
                "iphone-hd-upscaled/maps/DT_Phase3_ortho.png",
...
private struct BundleImage: View {
    ...
    var body: some View {
        if let image = Self.loadImage(from: candidates) {
            Image(uiImage: image)
                .resizable()
        } else {
            Rectangle()
    ...
    private static func loadImage(from candidates: [String]) -> UIImage? {
    ...
    private static func expandedCandidates(_ candidates: [String]) -> [String] {
        ...
        for candidate in candidates {
            ...
            if candidate.hasPrefix("LegacyAssets/") {
                appendCandidate(String(candidate.dropFirst("LegacyAssets/".count)))
            } else {
                appendCandidate("LegacyAssets/\(candidate)")
            }
        }
