...
Get-ChildItem -LiteralPath (Join-Path $projectRoot "ios-scaffold\Resources") -Directory | ForEach-Object {
    if ($_.Name -eq "LegacyAssets") {
        return
    }

    Copy-Item -LiteralPath $_.FullName -Destination (Join-Path $resourcesDestination $_.Name) -Recurse -Force
}
